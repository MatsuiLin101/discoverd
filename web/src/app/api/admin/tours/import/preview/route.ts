...
import { analyzeTours, readTourRows } from "@/lib/excel/tours";
...
export async function POST(req: NextRequest) {
  try {
    ...
    let rows;
    try {
      rows = await readTourRows(buf);
    } catch {
      return NextResponse.json({ error: "無法解析此 Excel 檔案" }, { status: 400 });
    }

    const { preview, payload } = await analyzeTours(rows);
    const prior = await findPriorImport("TOUR", md5);
    ...
    console.error("[POST /api/admin/tours/import/preview]", e);
