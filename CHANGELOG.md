...
- **行程彈窗一鍵關閉**：先前在行程內容彈窗連續點擊「相關行程」會逐層堆疊歷史，導致關閉時需點擊多次才能關掉。現在右上角 X（及 ESC、點背景）一次即可關閉並回到目前所在的列表，同時保留「相關行程」以 push 導航，瀏覽器上一頁／下一頁仍可在看過的行程間切換。關閉後下一頁不再出現無意義的「假切換」，按上一頁則會重新開啟剛關閉的行程。
